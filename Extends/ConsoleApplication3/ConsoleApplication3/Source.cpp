...
DllExport ushort FaceKit(ushort rows, ushort cols, int widthStep, byte *rgbData,
	ushort *countObjectBox, ushort *x0, ushort *y0, ushort *x1, ushort *y1, ushort *x2, ushort *y2, ushort *x3, ushort *y3);
...
DllExport short TestEast()
{
	...
	while (1)
	{
		...
		countObjectBox = 0;
		// Goi ham thu vien de lay dau ra la countTextBox, aVertices_x, aVertices_y
		ok = FaceKit((ushort)IplRGB.height, (ushort)IplRGB.width, IplRGB.widthStep, (byte*)IplRGB.imageData,
			&countObjectBox, x0, y0, x1, y1, x2, y2, x3, y3);
