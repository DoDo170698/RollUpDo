...
namespace ClientApp.Commons
{
    public class FileAndFolder
    {
        ...
        public static List<string> GetFiles(string pathFolder, string format = null)
        {
            //List<string> list = new List<string>();
            //DirectoryInfo d = new DirectoryInfo(pathFolder);
            //var f = d.GetFiles("*."+format);
            //foreach (var item in f)
            //{
            //    list.Add(item.FullName);
            //}
            //return list;
            return Directory.GetFiles(pathFolder, "*." + format, SearchOption.TopDirectoryOnly).ToList();
        }
