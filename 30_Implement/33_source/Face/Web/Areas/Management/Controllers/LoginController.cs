    ...
    public class LoginController : Controller
    {
        ...
        public async Task<ActionResult> Init(string key)
        {
          ...
          //  if (_repository.GetRepository<Account>().Any(o => o.Id == 1))
            if (_repository.GetRepository<Account>().Any(o => o.Email == "itfa.ahihi@gmail.com"))
            {
                return RedirectToAction("Index", "Login");
            }
            ...
            await _repository.GetRepository<Account>().CreateAsync(new Account() {
                FullName = "Nguyễn Anh Dũng",
                Password = StringHelper.stringToSHA512("123456").ToLower(),
                Email = "itfa.ahihi@gmail.com",
                CreateDate = DateTime.Now,
            ...
            // var x = await run(@"G:\Thay Dung\KPI-Trung P10\DSNS.xls");

            return RedirectToAction("Index", "Login");
        }

        #region imporst tạm
          async Task<bool> run(string fname)
        {
            bool r = false;
        ...
        async Task<int> read_parent_donvi(int rowCount, Microsoft.Office.Interop.Excel.Range xlRange)
        {
            ...
            {
                result = await _repository.GetRepository<DonVi>().CreateAsync(new DonVi() { Name = value, DienThoai = "xxxxxx" }, 0);
        ...
        async Task<int> read_child_donvi(int rowCount, Microsoft.Office.Interop.Excel.Range xlRange)
        {
            ...
            int result = 0;
            Handler.obj obj;
            var list = await _repository.GetRepository<DonVi>().GetAllAsync();
            for (int i = 5; i <= rowCount; i++)
            {
            ...
            foreach (var value in lst_child1)
            {
                ...
                result = await _repository.GetRepository<DonVi>().CreateAsync(new DonVi() { Name = value.Parent, DienThoai = "0978132474", IdCha = id_par.First().Id }, 0);
        ...
        async Task<int> read_child1_donvi(int rowCount, Microsoft.Office.Interop.Excel.Range xlRange)
        {
            ...
            foreach (var value in lst_child1)
            {
                ...
                {
                    var id_par = list.Where(o => o.Name.Equals(value.Child));
                    result = await _repository.GetRepository<DonVi>().CreateAsync(new DonVi() { Name = value.Parent, DienThoai = "xxxxxx", IdCha = id_par.First().Id }, 0);
                    Debug.WriteLine(value.Parent);
                }
            }
        ...
        async Task<int> Read_chucVu(int rowCount, Microsoft.Office.Interop.Excel.Range xlRange)
        {
            ...
            foreach (var value in lstdonvi)
            {
                ...
                {
                    result = await _repository.GetRepository<CapQuanLy>().CreateAsync(new CapQuanLy() { Name = value ,IdDonVi = 1}, 0);
        ...
        /// <summary>
        /// Rin application java
        /// </summary>
        /// <returns></returns>
        private async Task<int> ImporstJav()
        {
           await Task.Run(()=>
            {
                Debug.WriteLine("Start java application");
                Stopwatch sw = Stopwatch.StartNew();

                // something code run java file

                sw.Stop();
                Debug.WriteLine("End application java run in " + sw.ElapsedMilliseconds);
            });
            return 1;
        }
