...
namespace Web.Areas.Management.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        {
            // tìm đơn vị đó hăojc đơn vị cha có id== id truyền lên 
            ViewBag.IDCapQuanLy = new SelectList(await _repository.GetRepository<CapQuanLy>().GetAllAsync(o=>o.Id==1 || o.Id ==2), "Id", "Name", IdDonVi);
            if (IdDonVi!= null)
            {
                ViewBag.IdDonVi = new SelectList( await _repository.GetRepository<DonVi>().GetAllAsync(o => o.Id == IdDonVi || o.IdCha == IdDonVi), "Id", "Name", IdDonVi);
                return View();
            }
            ViewBag.IdDonVi = new SelectList(await _repository.GetRepository<DonVi>().GetAllAsync(), "Id", "Name", IdDonVi);
            return View();
        }
        /// <summary>
        /// Nhập tài khoản hệ thống mới
        ...
        public async Task<ActionResult> Create(Account model, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    //
                    account.IsManageAccount = false;
                    account.IDCapQuanLy = model.IDCapQuanLy;
                    ...
                    account.Password = StringHelper.stringToSHA512(password).ToLower();
        ...
        public async Task<ActionResult> Read(long id)
        {
            ...
            ViewBag.IdDonVi = new SelectList(await _repository.GetRepository<DonVi>().GetAllAsync(o => o.Id == IdDonVi || o.IdCha == IdDonVi), "Id", "Name", account.IdDonVi);
            ViewBag.IDCapQuanLy = new SelectList(await _repository.GetRepository<CapQuanLy>().GetAllAsync(o => o.IdDonVi == account.IdDonVi), "Id", "Name", account.IDCapQuanLy);
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (account == null)
                        return Json(new { success = false, message = "Không tìm thấy tài khoản người dùng!" }, JsonRequestBehavior.AllowGet);
                    string password = StringHelper.stringToSHA512( StringHelper.KillChars(model.OldPassword).ToLower());
                    string newPassword = StringHelper.stringToSHA512( StringHelper.KillChars(model.NewPassword).ToLower());
