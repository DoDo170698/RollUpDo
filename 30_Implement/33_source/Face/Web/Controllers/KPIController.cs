  ...
  public class KPIController : Controller
  {
	...
	public ActionResult Index()
    {
	    ...
	    var user = (from ac in _repository.GetRepository<Account>().GetAll()
							    join dv in _repository.GetRepository<DonVi>().GetAll() on ac.IdDonVi equals dv.Id
	    ...
	    var Kpi = (
		    ...
		    join uk in _repository.GetRepository<UserKPI>().GetAll() on ac.Id equals uk.IdUser
		    join kpi in _repository.GetRepository<KPI>().GetAll() on uk.IdKPI equals kpi.Id
    ...
    {
        var gr = _repository.GetRepository<KPI>().GetAll().Where(x => x.IdCha == id);
        foreach (var item in gr)
        {
		 ...
		 public static int get_all_chau(long id) {
							 int row = 0;
							 foreach (var value in _repository.GetRepository<KPI>().GetAll().Where(x => x.IdCha == id))
							 {
										foreach (var item in _repository.GetRepository<KPI>().GetAll().Where(x => x.IdCha == value.Id))
										{
												 foreach (var item_1 in _repository.GetRepository<KPI>().GetAll().Where(x => x.IdCha == item.Id))
												 {
															row++;
												 }
										}
		 ...
		 {
					var gr = (from uk in _repository.GetRepository<UserKPI>().GetAll()
										join kpi in _repository.GetRepository<KPI>().GetAll() on uk.IdKPI equals kpi.Id
										join w in _repository.GetRepository<Weight>().GetAll() on uk.IdWeight equals w.id
